...
namespace DBinv\Base;

use \Exception;
use \PDO;
use DBinv\productos as Childproductos;
use DBinv\productosQuery as ChildproductosQuery;
use DBinv\Map\productosTableMap;
 ...
 *
 *
 * @method     ChildproductosQuery orderById($order = Criteria::ASC) Order by the id column
 * @method     ChildproductosQuery orderByCod($order = Criteria::ASC) Order by the cod column
 * @method     ChildproductosQuery orderByNombre($order = Criteria::ASC) Order by the nombre column
 *
 * @method     ChildproductosQuery groupById() Group by the id column
 * @method     ChildproductosQuery groupByCod() Group by the cod column
 ...
 *
 * @method     Childproductos findOneById(int $id) Return the first Childproductos filtered by the id column
 * @method     Childproductos findOneByCod(int $cod) Return the first Childproductos filtered by the cod column
 ...
 *
 * @method     Childproductos requireOneById(int $id) Return the first Childproductos filtered by the id column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 * @method     Childproductos requireOneByCod(int $cod) Return the first Childproductos filtered by the cod column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     Childproductos[]|ObjectCollection findById(int $id) Return Childproductos objects filtered by the id column
 * @method     Childproductos[]|ObjectCollection findByCod(int $cod) Return Childproductos objects filtered by the cod column
...
abstract class productosQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \DBinv\Base\productosQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\DBinv\\productos', $modelAlias = null)
    {
    ...
    public function findPk($key, ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = productosTableMap::getInstanceFromPool(null === $key || is_scalar($key) || is_callable([$key, '__toString']) ? (string) $key : $key)))) {
    ...
    {
        $sql = 'SELECT id, cod, nombre FROM productos WHERE id = :p0';
        try {
        ...
        if ($row = $stmt->fetch(\PDO::FETCH_NUM)) {
            ...
            $obj->hydrate($row);
            productosTableMap::addInstanceToPool($obj, null === $key || is_scalar($key) || is_callable([$key, '__toString']) ? (string) $key : $key);
        }
        $stmt->closeCursor();
    ...
    public function filterByPrimaryKey($key)
    {
        ...
        return $this->addUsingAlias(productosTableMap::COL_ID, $key, Criteria::EQUAL);
    }
    ...
    public function filterByPrimaryKeys($keys)
    {
        ...
        return $this->addUsingAlias(productosTableMap::COL_ID, $keys, Criteria::IN);
    }
    ...
    public function filterById($id = null, $comparison = null)
    {
        if (is_array($id)) {
            $useMinMax = false;
            if (isset($id['min'])) {
                $this->addUsingAlias(productosTableMap::COL_ID, $id['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($id['max'])) {
                $this->addUsingAlias(productosTableMap::COL_ID, $id['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(productosTableMap::COL_ID, $id, $comparison);
    }

    /**
     * Filter the query on the cod column
     *
     * Example usage:
     * <code>
     * $query->filterByCod(1234); // WHERE cod = 1234
     * $query->filterByCod(array(12, 34)); // WHERE cod IN (12, 34)
     * $query->filterByCod(array('min' => 12)); // WHERE cod > 12
     * </code>
     *
     * @param     mixed $cod The value to use as filter.
     *              Use scalar values for equality.
     *              Use array values for in_array() equivalent.
     *              Use associative array('min' => $minValue, 'max' => $maxValue) for intervals.
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     ...
     * @return $this|ChildproductosQuery The current query, for fluid interface
     */
    public function filterByCod($cod = null, $comparison = null)
    {
        if (is_array($cod)) {
            $useMinMax = false;
            if (isset($cod['min'])) {
                $this->addUsingAlias(productosTableMap::COL_COD, $cod['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($cod['max'])) {
                $this->addUsingAlias(productosTableMap::COL_COD, $cod['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(productosTableMap::COL_COD, $cod, $comparison);
    }
    ...
    {
        if ($productos) {
            $this->addUsingAlias(productosTableMap::COL_ID, $productos->getId(), Criteria::NOT_EQUAL);
        }
